...
const FONT_URL: &str = "https://github.com/google/material-design-icons/raw/master/font/MaterialIcons-Regular.ttf";
...
fn main() {
    ...
    if !cp_path.exists() {
        if let Ok(src) = std::env::var("EGUI_SHADCN_CODEPOINTS_PATH") {
            fs::copy(&src, &cp_path)
                .expect("copy codepoints from EGUI_SHADCN_CODEPOINTS_PATH");
        } else {
            eprintln!(
                "[egui_components] downloading Material Icons codepoints…"
            );
    ...
    if let Ok(url) = std::env::var("EGUI_SHADCN_CUSTOM_FONT_URL") {
        ...
        if !custom_ttf.exists() {
            eprintln!(
                "[egui_components] downloading custom font '{font_name}'…"
            );
            match download_bytes(&url) {
                ...
                Err(e) => {
                    eprintln!(
                        "[egui_components] WARNING: could not download custom font: {e}"
                    );
                }
            }
        }
        ...
        if custom_ttf.exists() {
            ...
            fs::write(
                ...
                format!(
                    "pub const CUSTOM_FONT_NAME: &str = {:?};\n",
                    font_name
                ),
            )
    ...
    if !ttf_path.exists() {
        if let Ok(src) = std::env::var("EGUI_SHADCN_FONT_PATH") {
            fs::copy(&src, &ttf_path)
                .expect("copy font from EGUI_SHADCN_FONT_PATH");
        } else {
            ...
            match download_bytes(FONT_URL) {
                Ok(bytes) => {
                    ...
                    eprintln!(
                        "[egui_components] font saved → {}",
                        ttf_path.display()
                    );
                }
                Err(e) => {
                    eprintln!(
                        "[egui_components] WARNING: could not download font: {e}"
                    );
                }
            }
        }
