...
impl<'a> Table<'a> {
    ...
    pub fn show(
        ...
        let (table_rect, _) = ui.allocate_exact_size(Vec2::new(total_width, total_height), egui::Sense::hover());

        ui.painter()
            .rect_filled(table_rect, egui::CornerRadius::ZERO, theme.card);

        let header_rect = Rect::from_min_size(table_rect.min, Vec2::new(total_width, header_height));
        ui.painter()
        ...
        for (ci, col) in self.columns.iter().enumerate() {
            ...
            let cell_rect = Rect::from_min_size(Pos2::new(col_x, header_rect.top()), Vec2::new(cw, header_height));
        ...
        for row_idx in 0..rows {
            ...
            let row_rect =
                Rect::from_min_size(Pos2::new(table_rect.left(), row_top), Vec2::new(total_width, row_height));
