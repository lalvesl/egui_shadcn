...
use crate::{ShadcnTheme, ICON_EXPAND_MORE};
...
impl<'a> Select<'a> {
    pub fn new(current: &'a mut Option<usize>, options: &'a [&'a str]) -> Self {
        Self { current, options, placeholder: "Select…", width: None }
    }
    ...
    pub fn show(self, ui: &mut Ui) -> bool {
        ...
        let border_color = if resp.hovered() { theme.ring } else { theme.border };

        ui.painter().rect_filled(trigger_rect, cr, theme.background);
        ui.painter().rect_stroke(trigger_rect, cr, Stroke::new(1.0, border_color), egui::StrokeKind::Inside);

        let display    = self.current.map(|i| self.options[i]).unwrap_or(self.placeholder);
        let text_color = if self.current.is_some() { theme.foreground } else { theme.muted_foreground };
        ...
        let is_open = ui.ctx().data(|d| d.get_temp::<bool>(popup_id).unwrap_or(false));
        ...
        if is_open {
                    ...
                    Frame::new()
                        ...
                        .show(ui, |ui| {
                            ...
                            for (i, opt) in self.options.iter().enumerate() {
                                ...
                                let text_col = if is_sel { theme.accent_foreground } else { theme.foreground };
            ...
            if ui.ctx().input(|i| i.pointer.any_click()) {
                ...
                if !trigger_rect.contains(ui.ctx().input(|i| i.pointer.hover_pos().unwrap_or_default()))
                    && !popup_rect.contains(ui.ctx().input(|i| i.pointer.hover_pos().unwrap_or_default()))
                {
                    ui.ctx().data_mut(|d| d.insert_temp(popup_id, false));
                }
            }
