...
impl<'t> Tabs<'t> {
    pub fn new(id: impl std::hash::Hash, labels: &'t [&'t str], current: &'t mut usize) -> Self {
        Self { id: egui::Id::new(id), labels, current }
    }
    ...
    pub fn show(self, ui: &mut Ui, mut content: impl FnMut(&mut Ui, usize)) {
        ...
        let widths: Vec<f32> = self.labels.iter().map(|lbl| {
            let g = ui.painter().layout_no_wrap((*lbl).to_owned(), font.clone(), Color32::WHITE);
            g.size().x + tab_pad_h * 2.0
        }).collect();

        let bar_rect_alloc = ui.allocate_exact_size(
            Vec2::new(available_w, bar_h),
            Sense::hover(),
        ).0;
        ...
        for (i, (lbl, w)) in self.labels.iter().zip(widths.iter()).enumerate() {
            ...
            let (tab_bg, text_color) = if is_active {
                ...
                (ShadcnTheme::with_alpha(theme.background, 80), theme.foreground)
            } else {
            ...
            painter.rect_filled(tab_rect, CornerRadius::same((theme.radius - 2.0) as u8), tab_bg);
