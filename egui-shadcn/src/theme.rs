...
fn foreground_for(bg: Color32) -> Color32 {
    if luminance(bg) > 0.3 { hsl(0.0, 0.0, 0.08) } else { hsl(0.0, 0.0, 0.98) }
}
...
impl ShadcnTheme {
    pub fn light() -> Self  { Self::build(false, None) }
    pub fn dark()  -> Self  { Self::build(true,  None) }
    ...
    pub fn apply(&self, ctx: &Context) {
        let mut vis = if self.dark { Visuals::dark() } else { Visuals::light() };
